...
export async function createAuditServer({ token, logPath, host = '127.0.0.1', port = 8788 }) {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && request.url === '/health') return reply(200, { ok: true, sequence: last?.sequence || 0, lastHash: last?.hash || null });
    ...
    try {
      ...
      writeQueue = writeQueue.then(async () => {
        const record = { sequence: (last?.sequence || 0) + 1, storedAt: new Date().toISOString(), previousHash: last?.hash || null, payload };
        record.hash = createHash('sha256').update(`${record.previousHash || 'GENESIS'}:${stableStringify(record.payload)}:${record.storedAt}:${record.sequence}`).digest('hex');
...
if (isMain) {
  const server = await createAuditServer({
    ...
    port: Number(process.env.AUDIT_SINK_PORT || 8788)
  });
